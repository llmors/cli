...
final class RunCommand extends AbstractManifestCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $arguments = $this->mergeKeyValues($input->getOption('input'), $input->getOption('arg'));
            $config = $this->mergeKeyValues($input->getOption('config-json'), $input->getOption('config'));
    ...
    private function listFunctions(OutputStyle $io, OutputInterface $output, bool $json): int
    {
        ...
        foreach ($manifest->functions as $function) {
            $rows[] = [$function->functionKey, $function->runtime, OutputStyle::oneLine($function->description, 60)];
        }
